...
.controller('ApplicationCtrl', ['$rootScope', '$scope', '$http', '$location', '$window', function ($rootScope, $scope, $http, $location, $window) {
		// Check for token in the local HTML5 client side storage
...
.controller('ProfileCtrl', ['$rootScope', '$scope', '$http', '$location', '$window', function ($rootScope, $scope, $http, $location, $window) { // Retrieve a NewsWatcher user profile
		$http({
			...
			for (var i = 0; i < $scope.user.filters.length; i++) {
				$scope.user.filters[i].keywordsStr = $scope.user.filters[i].keyWords.join(',');
			}
		...
		$scope.deleteFilter = function () {
			$scope.user.filters.splice($scope.selectedIdx, 1);
			$scope.selectedIdx = 0;
		}
		
		$scope.addFilter = function () {
			if ($scope.user.filters.length == 5) {
				$scope.$emit('msg', "No more filters allowed");
			} else {
				$scope.user.filters.push({
					name: 'New Filter',
					keyWords : ["Keyword"],
					keywordsStr : "Keyword",
					enableAlert : false,
				...
				$scope.selectedIdx = $scope.user.filters.length - 1;
			}
		}
...
.controller('NewsCtrl', ['$rootScope', '$scope', '$http', '$routeParams', function ($rootScope, $scope, $http, $routeParams) { // Retrieve NewsWatcher news
		...
		$http({
			...
			if ($routeParams.saved) {
				...
				$scope.news = $scope.user.filters[$scope.selectedIdx].newsStories;
				for (var i = 0; i < $scope.user.filters.length; i++) {
					for (var j = 0; j < $scope.user.filters[i].newsStories.length; j++) {
						$scope.user.filters[i].newsStories[j].hours = toHours($scope.user.filters[i].newsStories[j].date);
					}
				}
			}
